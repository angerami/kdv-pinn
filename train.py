...
def extract_losses_MSE(results, config):
    ...
    for k in loss_types:
        ...
        losses[f'L_{k}'] = lambda_k * L_k.detatch().cpu().item()
    return L_total, losses



def pretrain(model, target, config, device, num_epochs=200):
...
def train_pinn(model, config, device, start_epoch=0, optimizer_states=None, interactive=False):
    ...
    #Plot initialization
    plot_interval = getattr(config, 'plot_interval', max(1, config.plot_interval))
    ...
    fcn = soliton_library[config.soliton_type]
    ...
    for epoch in range(start_epoch, config.num_epochs):
        ...
        loss, metrics = extract_losses_MSE(results) 
        metrics.update(extract_means(results))
        ...
        if epoch % plot_interval == 0:
            print(f'Epoch {epoch:4d} | ' + ' | '.cat([f'{k} : {v:.3e} ' for k, v in loss_dict.items()]))
            if interactive:
